...
public abstract class DataTypeConversion {
    ...
    public static Conversion conversionFor(DataType from, DataType to) {
        ...
        if (conversion == null) {
//            throw new ZoneIllegalArgumentException("cannot convert from [" + from + "] to [" + to + "]");
        }
        return conversion;
    }
    ...
    public static byte safeToByte(long x) {
        if (x > Byte.MAX_VALUE || x < Byte.MIN_VALUE) {
            throw new PlainIllegalArgumentException("[" + x + "] out of [byte] range");
        }
        return (byte) x;
    }
    ...
    public static short safeToShort(long x) {
        if (x > Short.MAX_VALUE || x < Short.MIN_VALUE) {
            throw new PlainIllegalArgumentException("[" + x + "] out of [short] range");
        }
        return (short) x;
    }
    ...
    public static int safeToInt(long x) {
        if (x > Integer.MAX_VALUE || x < Integer.MIN_VALUE) {
            throw new PlainIllegalArgumentException("[" + x + "] out of [integer] range");
        }
        return (int) x;
    }
    ...
    public static long safeToLong(double x) {
        if (x > Long.MAX_VALUE || x < Long.MIN_VALUE) {
            throw new PlainIllegalArgumentException("[" + x + "] out of [long] range");
        }
        return Math.round(x);
    }
    ...
    public static boolean convertToBoolean(String val) {
        ...
        if (Booleans.isBoolean(lowVal) == false) {
            throw new PlainIllegalArgumentException("cannot cast [" + val + "] to [boolean]");
        }
     ...
     * <p>
     * Throws ZoneIllegalArgumentException if such conversion is not possible
     */
    ...
    public enum Conversion {
        ...
        DATETIME_TO_STRING(o -> DateUtils.toDateString((ZonedDateTime) o)),
        ...
        STRING_TO_IP(o -> {
            if (!InetAddresses.isInetAddress(o.toString())) {
                throw new PlainIllegalArgumentException( "[" + o + "] is not a valid IPv4 or IPv6 address");
            }
            return o;
        });
        ...
        private static Function<Object, Object> fromString(Function<String, Object> converter, String to) {
            return (Object value) -> {
                try {
                    ...
                    throw new PlainIllegalArgumentException("cannot cast [{}] to [{}]", value, to);
                } catch (IllegalArgumentException e) {
                    throw new PlainIllegalArgumentException("cannot cast [{}] to [{}]:{}", value, to, e.getMessage());
                }
            };
        }
